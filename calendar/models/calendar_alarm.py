...
class Alarm(models.Model):
    ...
    _interval_selection = {'minutes': 'Phút', 'hours': 'Giờ', 'days': 'Ngày'}

    name = fields.Char('Name', required=True)
    # , ('email', 'Email')
    alarm_type = fields.Selection(
        [('notification', 'Thông báo hệ thống')],
        string='Type', required=True, default='notification')
    ...
    def _onchange_duration_interval(self):
        display_interval = dict(self._fields['interval'].selection).get(self.interval, '')
        display_alarm_type = {
